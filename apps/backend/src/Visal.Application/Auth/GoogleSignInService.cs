...
public interface IGoogleSignInService
{
    ...
    /// <summary>
    /// Intercambia el code y resuelve el usuario de VISAL. Si el correo no existe y se entrega
...
/// <summary>
/// Login con Google (OIDC). Google confirma la identidad; VISAL decide el acceso: el correo debe
    ...
    public async Task<GoogleSignInResult> ResolveAsync(string code, string redirectUri, string? signupAgencyName = null, CancellationToken cancellationToken = default)
    {
        ...
        if (user is null || user.Status != PlatformUserStatus.Active)
        {
            return new GoogleSignInResult(false,
                "Tu cuenta de Google fue validada, pero todavia no tienes acceso a VISAL.travels. Solicita una invitacion al administrador.");
        }
