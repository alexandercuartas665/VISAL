# Rework-PPFO37-DesescalonamientoPAD.ps1
# Reconstruye PP-FO-37 CONSENTIMIENTO DE DESESCALONAMIENTO Y/O EGRESO PAD.
...
$raw = docker exec $PgContainer psql -U $PgUser -d $PgDb -tA -c "SELECT schema_json::text FROM form_definitions WHERE codigo='PP-FO-37' AND tenant_id='$TenantId';"
...
$def_general = "Desescalonamiento del PAD y/o egreso del Programa de atención domiciliaria se informa de manera clara y concisa a paciente y/o familiares que usuario(a) requiere continuar manejo médico y plan terapéutico según corresponda de manera presencial en la Microred de su EPS, se dan recomendaciones médicas y signos de alarma, se orienta en las rutas de servicios de salud de su Microred."
...
$secEncabezado = @{
    ...
    children = @(
        (Field "CIUDAD"           "ciudad_encabezado_deses" "text" 4),
        (Field "FECHA DE ATENCIÓN" "fecha_atencion_deses"    "date" 4),
        (Field "HORA DE ATENCIÓN"  "hora_atencion_deses"     "text" 4)
    )
}
...
$secDeclaracion = @{
    ...
    children = @(
        (Field "Yo (nombre)"        "nombre_declarante_deses" "text" 8),
        (Field "Tipo de documento"  "tipo_doc_declarante"     "select" 2 @{ options = @("CC","CE","TI","PP","Otro"); defaultValue = "CC"; allowCustom = $true }),
        (Field "Número"             "num_doc_declarante"      "text" 2),
        (Field "Expedido en la ciudad de" "ciudad_expedicion_deses" "text" 6),
        (Field "En condición de"    "condicion_declarante"    "select" 6 @{ options = @("Paciente","Familiar","Otro"); defaultValue = "Paciente"; allowCustom = $true }),
        (Field "Otro (especifique)" "condicion_otro"          "text" 12),
        (P $decl_final)
    )
}
...
$secFirmasManu = @{
    ...
    children = @(
        (Field "Firma del Paciente y/o Cuidador Primario" "firma_paciente_cuidador" "text" 8),
        (Field "CC del Paciente/Cuidador"                 "cc_paciente_cuidador"    "text" 4),
        (P "Profesional que realiza egreso del programa de atención domiciliaria"),
        (Field "Nombre del Profesional" "nombre_profesional_deses" "text" 6),
        (Field "Firma del Profesional"  "firma_profesional_deses"  "text" 6)
    )
}
...
$sql = "UPDATE form_definitions SET schema_json='$jsonSql'::jsonb, updated_at='$now' WHERE codigo='PP-FO-37' AND tenant_id='$TenantId';"
$tmp = [System.IO.Path]::GetTempFileName()
[System.IO.File]::WriteAllText($tmp, $sql, [System.Text.UTF8Encoding]::new($false))
try {
    $copy = "/tmp/visal_pp37_$([Guid]::NewGuid().ToString('N')).sql"
...
Write-Host "OK PP-FO-37 actualizado." -ForegroundColor Green
