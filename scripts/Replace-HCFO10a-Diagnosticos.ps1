# Replace-HCFO10a-Diagnosticos.ps1
# Reemplaza UNICAMENTE los 3 campos planos CIE / ORIGEN / RELACION del
# Encabezado de HC-FO-10a por una tabla de Diagnosticos con la MISMA
# estructura que la usada en HC-FO-08 (autocomplete cie11 + autocomplete
# origen + select tipo de diagnostico). Conserva todo lo demas. NO TOCA
# HC-FO-08.
...
# 3) Localizar EXACTAMENTE los 3 campos por name (cie / origen / relacion).
# Deben ser consecutivos. Si no lo son, abortamos antes de hacer dano.
...
# 4) Construir tabla con la MISMA estructura que la de HC-FO-08.
# Columnas (en orden):
#   1) Diagnostico  : autocomplete cie11
#   2) Origen       : autocomplete (sin catalog)
#   3) Tipo de diagnostico principal : select con 3 opciones
$cols = @(
    @{
        id = newId; label = "Diagnostico"; name = "diagnostico"
        fieldType = "autocomplete"; catalog = "cie11"
        allowCustom = $false; defaultValue = ""
    },
    @{
        id = newId; label = "Origen"; name = "origen"
        fieldType = "autocomplete"; catalog = ""
        allowCustom = $false; defaultValue = ""
    },
    @{
...
# 5) Reemplazar [start..end] por $nuevo
$head = @(); $tail = @()
...
try {
    $copy = "/tmp/visal_dx_$([Guid]::NewGuid().ToString('N')).sql"
