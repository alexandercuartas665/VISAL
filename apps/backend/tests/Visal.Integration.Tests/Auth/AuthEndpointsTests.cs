...
public sealed class AuthEndpointsTests : IClassFixture<VisalApiFactory>
{
    private readonly VisalApiFactory _factory;

    public AuthEndpointsTests(VisalApiFactory factory) => _factory = factory;

    [Fact]
    public async Task Login_SingleTenantUser_ReturnsTokenWithTenant()
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, VisalApiFactory.SingleEmail);
    ...
    public async Task Login_WrongPassword_Returns401()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/connect/token",
            new LoginRequest(VisalApiFactory.SingleEmail, "clave-incorrecta"));
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, VisalApiFactory.MultiEmail);
    ...
    public async Task TenantConfigurations_AreIsolatedByTokenTenant()
    {
        ...
        var tokenA = await LoginAsync(client, VisalApiFactory.SingleEmail);
        ...
        var tokenMulti = await LoginAsync(multi, VisalApiFactory.MultiEmail);
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, VisalApiFactory.SuperEmail);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
    ...
    {
        var response = await client.PostAsJsonAsync("/connect/token", new LoginRequest(email, VisalApiFactory.Password, tenantId));
        response.EnsureSuccessStatusCode();
