# Rework-PPFO89-MedInterna.ps1
# Reconstruye PP-FO-89 CONSENTIMIENTO INFORMADO MEDICINA INTERNA segun docx.
...
# ================ SeedRows ================
$rowsProc = @(
    @("Atención médica especializada en el domicilio del paciente, orientada a la prevención, diagnóstico, tratamiento y seguimiento de enfermedades crónicas y agudas, de acuerdo con la condición clínica del usuario.", ""),
    @("Diagnóstico y prescripción de tratamientos médicos conforme a la pertinencia clínica.", ""),
    @("Solicitud, interpretación y evaluación de exámenes de laboratorio y ayudas diagnósticas.", ""),
...
$rowsBenef = @(
    @("Valoración integral del estado de salud por parte del médico especialista en Medicina Interna.", ""),
    @("Identificación oportuna de enfermedades agudas y crónicas.", ""),
    @("Establecimiento de diagnósticos clínicos basados en la evaluación médica y ayudas diagnósticas pertinentes", ""),
    @("Formulación y ajuste de tratamientos de acuerdo con la condición clínica del paciente.", ""),
    @("Seguimiento de la evolución de enfermedades y respuesta a los tratamientos instaurados.", ""),
    @("Prevención, detección y control de posibles complicaciones asociadas a la enfermedad.", ""),
    @("Orientación y educación al paciente, familiares y/o cuidadores sobre el manejo de su condición de salud.", ""),
    @("Mejoramiento de la calidad de vida mediante el control y manejo integral de las enfermedades identificadas.", "")
)
...
$rowsRiesgos = @(
    ...
    @("No adherencia al tratamiento por parte del paciente, familiar o cuidador, con impacto en los resultados esperados.", "")
)
...
$raw = docker exec $PgContainer psql -U $PgUser -d $PgDb -tA -c "SELECT schema_json::text FROM form_definitions WHERE codigo='PP-FO-89' AND tenant_id='$TenantId';"
...
# ================ Secciones ================
$secFecha = @{ id = newId; type = "section"; label = "Encabezado"; children = @( (Field "FECHA" "fecha_mi" "date" 4) ) }

$secInfo = @{
    id = newId; type = "section"; label = "2. INFORMACIÓN SOBRE EL PROCEDIMIENTO"
    children = @(
        (P "Marque con X el procedimiento a realizar"),
        (SH "PROCEDIMIENTO: ATENCIÓN EN MEDICINA INTERNA"),
...
$sql = "UPDATE form_definitions SET schema_json='$jsonSql'::jsonb, updated_at='$now' WHERE codigo='PP-FO-89' AND tenant_id='$TenantId';"
$tmp = [System.IO.Path]::GetTempFileName()
[System.IO.File]::WriteAllText($tmp, $sql, [System.Text.UTF8Encoding]::new($false))
try {
    $copy = "/tmp/visal_pp89_$([Guid]::NewGuid().ToString('N')).sql"
...
Write-Host "OK PP-FO-89 actualizado." -ForegroundColor Green
