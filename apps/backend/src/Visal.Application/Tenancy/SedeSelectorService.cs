...
public sealed class SedeSelectorService(IApplicationDbContext db) : ISedeSelectorService
{
    public async Task<IReadOnlyList<SucursalDto>> GetSedesAsync(Guid platformUserId, Guid tenantId, CancellationToken ct = default)
    {
        ...
        if (tu is null)
        {
            ...
            return todas.Select(s => new SucursalDto(s.Id, s.Codigo, s.Nombre, s.Direccion, s.Ciudad, s.Telefono, s.Activo, s.MipresObligatorio, s.CodigoHabilitacion)).ToList();
        }
        ...
        return todas.Where(s => set.Contains(s.Id))
            .Select(s => new SucursalDto(s.Id, s.Codigo, s.Nombre, s.Direccion, s.Ciudad, s.Telefono, s.Activo, s.MipresObligatorio, s.CodigoHabilitacion))
            .ToList();
    }
