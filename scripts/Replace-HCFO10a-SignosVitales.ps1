# Replace-HCFO10a-SignosVitales.ps1
# Reemplaza UNICAMENTE el subheading "SIGNOS VITALES" + sus 10 campos viejos
# dentro de "Encabezado del documento" en HC-FO-10a, por los nuevos campos
# con calculos (tensionClass / imc / imcClass / perimetroRiesgo). Conserva
# todo lo demas EXACTAMENTE como esta. NO TOCA HC-FO-08.
...
# 2) Localizar la seccion "Encabezado del documento"
$encIndex = -1
for ($i = 0; $i -lt $schema.children.Count; $i++) {
    if ($schema.children[$i].label -eq "Encabezado del documento") { $encIndex = $i; break }
}
if ($encIndex -lt 0) { throw "No encontre 'Encabezado del documento' en $Codigo." }
$enc = $schema.children[$encIndex]

# 3) Dentro del encabezado, localizar el rango [start..end] del subheading
# "SIGNOS VITALES" (incluye el subheading + sus campos hasta el proximo subheading)
$startIdx = -1; $endIdx = -1
...
if ($startIdx -lt 0) { throw "No encontre subheading 'SIGNOS VITALES'." }
if ($endIdx -lt 0) { $endIdx = $enc.children.Count - 1 }
$oldCount = $endIdx - $startIdx + 1
Write-Host "    Rango SIGNOS VITALES actual: indices [$startIdx..$endIdx] = $oldCount nodos" -ForegroundColor Cyan

# 4) Construir el nuevo bloque (subheading + 15 nodos con calculos),
# para vivir dentro del Encabezado como hermanos de los otros subheadings.
$newBlock = @(
...
# 5) Construir nueva lista de children = head + newBlock + tail
$head = @()
$tail = @()
...
Write-Host "    Sustituidos $oldCount nodos -> $($newBlock.Count) nodos nuevos" -ForegroundColor Green
Write-Host "    Hijos totales del Encabezado: $($enc.children.Count) (era 82)"
...
try {
    $copy = "/tmp/visal_sv_hcfo10a_$([Guid]::NewGuid().ToString('N')).sql"
...
Write-Host "Solo se toco el bloque SIGNOS VITALES. Todo lo demas IDENTICO." -ForegroundColor Yellow
