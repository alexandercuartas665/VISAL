# Rework-PPFO20-MedGeneral.ps1
# Reconstruye PP-FO-20 CONSENTIMIENTO MEDICINA GENERAL segun docx fiel.
...
# ================ SeedRows ================
$rowsProc = @(
    @("Atención médica para prevención, detección tratamiento y seguimiento de las enfermedades crónicas estabilizadas.", ""),
    @("Diagnóstico y prescripción de tratamientos médicos a los pacientes", ""),
    @("Remisión a consulta especializada cuando el caso lo amerite", ""),
    @("Evaluación de exámenes de laboratorio, radiografías, tratamiento e indicaciones médicas.", ""),
    @("Seguimiento a los tratamientos de los pacientes, a través de consultas fijadas", "")
)

$rowsBenef = @(
    @("Valoración médica integral del estado de salud del usuario.", ""),
    @("Identificación temprana de enfermedades o alteraciones clínicas.", ""),
    @("Seguimiento y control de patologías agudas o crónicas.", ""),
    @("Formulación de tratamiento médico conforme a condición clínica.", ""),
    @("Orientación médica y educación al paciente y/o cuidador.", "")
)
...
$rowsRiesgos = @(
    ...
    @("Necesidad de estudios complementarios para confirmación diagnóstica.", "")
)

$rowsAltern = @(
    @("Continuar manejo por médico tratante de EPS.", ""),
    @("Valoración por medicina especializada.", ""),
    @("Atención por servicio de urgencias en caso de signos de alarma.", ""),
    @("Solicitud de segunda opinión médica cuando aplique.", "")
)
...
$rowsConsec = @(
    ...
    @("Retraso en inicio o continuidad del tratamiento.", "")
)

# ================ Cargar y localizar auto sections ================
$raw = docker exec $PgContainer psql -U $PgUser -d $PgDb -tA -c "SELECT schema_json::text FROM form_definitions WHERE codigo='PP-FO-20' AND tenant_id='$TenantId';"
...
$secInfo = @{
    ...
    children = @(
        ...
        (SH "PROCEDIMIENTO: ATENCIÓN EN MEDICINA GENERAL"),
        (Tabla "Atención en Medicina General" "procedimientos" $colsProc $rowsProc $true),
...
$sql = "UPDATE form_definitions SET schema_json='$jsonSql'::jsonb, updated_at='$now' WHERE codigo='PP-FO-20' AND tenant_id='$TenantId';"
$tmp = [System.IO.Path]::GetTempFileName()
[System.IO.File]::WriteAllText($tmp, $sql, [System.Text.UTF8Encoding]::new($false))
try {
    $copy = "/tmp/visal_pp20_$([Guid]::NewGuid().ToString('N')).sql"
...
Write-Host "OK PP-FO-20 actualizado." -ForegroundColor Green
