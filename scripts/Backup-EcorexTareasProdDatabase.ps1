<#
.SYNOPSIS
    Baja un respaldo logico (pg_dumpall) del postgres de PRODUCCION de ECOREX.tareas
    conectando por SSH al server remoto y guarda el archivo comprimido en
    D:\Backups\Produccion\Ecorex.tareas, con retencion por dias.

.DESCRIPTION
    Espejo de Backup-VisalProdDatabase.ps1 para el proyecto ECOREX.tareas (mismo server,
    misma llave SSH). Cambia el contenedor y el usuario de postgres:
    - SSH al server prod (root@10.0.0.3 con la llave id_ed25519_visal).
    - Ejecuta `docker exec ecorex-postgres-prod pg_dumpall -U ecorex` dentro
      del servidor y comprime el stream con gzip antes de bajarlo.
    - Guarda el .sql.gz en D:\Backups\Produccion\Ecorex.tareas\{yyyy-MM-dd_HHmmss}\.
    - No escribe passwords: pg_dumpall se autentica por trust local dentro
      del propio contenedor postgres.
...
.NOTES
    ...
    y el host key de 10.0.0.3 aceptado en known_hosts. El usuario 'ecorex' es
    superusuario del contenedor (POSTGRES_USER de la imagen oficial), asi que
    pg_dumpall captura roles + todas las bases, igual que en Visal.
#>

[CmdletBinding()]
param(
    [string]$BackupRoot     = "D:\Backups\Produccion\Ecorex.tareas",
    [string]$RemoteHost     = "10.0.0.3",
    [string]$RemoteUser     = "root",
    [string]$SshKey         = "$env:USERPROFILE\.ssh\id_ed25519_visal",
    [string]$ContainerName  = "ecorex-postgres-prod",
    [string]$PgUser         = "ecorex",
