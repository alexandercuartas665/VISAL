...
public sealed class AdminEndpointsTests : IClassFixture<VisalApiFactory>
{
    private readonly VisalApiFactory _factory;

    public AdminEndpointsTests(VisalApiFactory factory) => _factory = factory;
    ...
    private async Task<HttpClient> SuperAdminClientAsync()
    {
        var client = _factory.CreateClient();
        var response = await client.PostAsJsonAsync("/connect/token",
            new LoginRequest(VisalApiFactory.SuperEmail, VisalApiFactory.Password));
